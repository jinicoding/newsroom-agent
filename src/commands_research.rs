//! Research & analysis command handlers (리서치·분석·검증 도메인)
//! Commands: /clip, /factcheck, /glossary, /law, /network, /news, /press, /research, /sns, /sources, /trend
//!
//! Sourcing & monitoring commands (/alert, /contact, /follow, /monitor, /note, /rss, /tip,
//! /verify, /wire) moved to commands_sourcing.rs on Day 14. Re-exported here for backward compat.
...
// Re-export sourcing commands for backward compatibility
pub use crate::commands_sourcing::*;
...
pub(crate) fn clip_file_path(url: &str, date: &str) -> std::path::PathBuf {
...
pub(crate) fn save_clip(path: &std::path::Path, url: &str, content: &str) -> std::io::Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
...
pub(crate) fn json_extract_string(obj: &str, key: &str) -> Option<String> {
...
// ── /jsearch, /bigkinds, /dart, /assembly → commands_data.rs
pub use crate::commands_data::{
    handle_assembly, handle_bigkinds, handle_dart, handle_jsearch,
    ASSEMBLY_SUBCOMMANDS, BIGKINDS_SUBCOMMANDS, DART_SUBCOMMANDS,
};
